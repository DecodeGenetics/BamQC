                 ...
                 std::vector<int> klist)
{
    for (std::map<seqan::CharString, unsigned>::iterator it = laneNames.begin(); it != laneNames.end(); ++it)
    {
        ...
        outFile << "As_by_position_second"; printString(counts[lid].r2.dnacount[0], outFile);
        outFile << "Cs_by_position_first"; printString(counts[lid].r1.dnacount[1], outFile);
        outFile << "Cs_by_position_second"; printString(counts[lid].r2.dnacount[1], outFile);
        outFile << "Gs_by_position_first"; printString(counts[lid].r1.dnacount[2], outFile);
        outFile << "Gs_by_position_second"; printString(counts[lid].r2.dnacount[2], outFile);
        outFile << "Ts_by_position_first"; printString(counts[lid].r1.dnacount[3], outFile);
        outFile << "Ts_by_position_second"; printString(counts[lid].r2.dnacount[3], outFile);
